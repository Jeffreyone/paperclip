...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const projectName = (id: string | null) => {
    if (!id) return id?.slice(0, 8) ?? t("issue.noProject");
  ...
  const labelsContent = (
    <>
            ...
            return (
              <div key={label.id} className="flex items-center gap-1">
                ...
                <button
                  ...
                  title={t("issue.labelDelete", { name: label.name })}
                >
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        >
          {createLabel.isPending ? t("issue.creating") : t("issue.createLabel")}
        </button>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? t("issue.assignTo", { name: creatorUserLabel }) : t("issue.assignToRequester")}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
            ...
            <div className="flex items-center justify-between gap-3 rounded-md border border-border px-2 py-1.5 w-full">
              <div className="min-w-0">
                <div className="text-sm">
                  {usesIsolatedExecutionWorkspace ? t("issue.isolatedCheckout") : t("issue.projectPrimary")}
                </div>
