...
function getStatusLabelKey(status: string): string {
  const mapping: Record<string, string> = {
    ...
    "todo": "issue.statusTodo",
    "in_progress": "issue.statusInProgress",
    "in_review": "issue.statusInReview",
    "done": "issue.statusDone",
    "cancelled": "issue.statusCancelled",
    "blocked": "issue.statusBlocked",
  };
  return mapping[status] || `issue.status${status.charAt(0).toUpperCase() + status.slice(1).replace(/_([a-z])/g, (_, c) => c.toUpperCase())}`;
}
...
const quickFilterPresets = [
  { label: "All", statuses: [] as string[], labelKey: "filter.all" },
  { label: "Active", statuses: ["todo", "in_progress", "in_review", "blocked"], labelKey: "filter.active" },
  { label: "Backlog", statuses: ["backlog"], labelKey: "filter.backlog" },
  { label: "Done", statuses: ["done", "cancelled"], labelKey: "filter.done" },
];
...
export function IssuesList({
  ...
  const statusLabels = useMemo(() => {
    ...
    for (const s of statusOrder) {
      labels[s] = t(getStatusLabelKey(s));
    }
    return labels;
  }, [t]);
  ...
  const priorityLabels = useMemo(() => {
    ...
    for (const p of priorityOrder) {
      labels[p] = t(`issue.priority${p.charAt(0).toUpperCase() + p.slice(1)}`);
    }
    return labels;
  }, [t]);
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return statusOrder
        ...
        .map((s) => ({ key: s, label: statusLabels[s], items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return priorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: statusLabels[p], items: groups[p]! }));
    }
    // assignee
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t("filter.user"))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <Button size="sm" variant="outline" onClick={() => openNewIssue(newIssueDefaults())}>
            ...
            <span className="hidden sm:inline">{t("issue.newIssue")}</span>
          </Button>
          <div className="relative w-48 sm:w-64 md:w-80">
            ...
            <Input
              ...
              placeholder={t("common.search") + " " + t("nav.issues").toLowerCase() + "..."}
              className="pl-7 text-xs sm:text-sm"
              aria-label={t("common.search") + " " + t("nav.issues").toLowerCase()}
            />
          </div>
        </div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t("filter.filters")}: ${activeFilterCount}` : t("filter.filter")}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">{t("filter.quickFilters")}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("filter.priority")}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                    ...
                    {/* Assignee */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("filter.assignee")}</span>
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t("filter.noAssignee")}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("filter.labels")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ...
                    ["created", t("filter.created")],
                    ["updated", t("filter.updated")],
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("filter.group")}</span>
                </Button>
              </PopoverTrigger>
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("filter.noIssuesMatch")}
          action={t("issue.newIssue")}
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs">
                                ...
                                {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("filter.user")}
                              </span>
                            ) : (
