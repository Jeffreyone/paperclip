...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState }: ProjectPropertiesProps) {
  ...
  const clearLocalWorkspace = (workspace: Project["workspaces"][number]) => {
    const confirmed = window.confirm(
      workspace.repoUrl
        ? t("project.clearLocalFolderFromWorkspaceConfirm")
        : t("project.deleteWorkspaceLocalFolderConfirm"),
    );
    if (!confirmed) return;
  ...
  const clearRepoWorkspace = (workspace: Project["workspaces"][number]) => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("project.clearGitHubRepoFromWorkspaceConfirm")
        : t("project.deleteWorkspaceRepoConfirm"),
    );
    if (!confirmed) return;
    if (hasLocalFolder) {
      updateWorkspace.mutate({
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("common.name")} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("common.description")} state={fieldState("description")} />}
          alignStart
                ...
                >
                  ...
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={t("project.removeGoal", { title: goal.title })}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  )}
        ...
        <PropertyRow label={<FieldLabel label={t("common.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("common.updated")} state="idle" />}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("project.workspacesHelpLabel")}
                >
                  ?
                </button>
              </TooltipTrigger>
                        ...
                        >
                          <div className="min-w-0 space-y-0.5">
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {service.status === "running"
                                  ? t("project.serviceRunning")
                                  : service.status === "failed"
                                    ? t("project.serviceFailed")
                                    : service.status}
                              </span>
                            </div>
                            <div className="text-[11px] text-muted-foreground">
                              {service.url ? (
                                ...
                                service.command ?? t("project.noUrl")
                              )}
                            </div>
                          </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={t("project.executionWorkspacesHelpLabel")}
                >
                  ?
                </button>
              </TooltipTrigger>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2 text-sm font-medium">
                  <span>{t("project.enableIsolatedIssueCheckouts")}</span>
                  <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                </div>
                <div className="text-xs text-muted-foreground">
                  {t("project.enableIsolatedIssueCheckoutsDesc")}
                </div>
              </div>
              {onUpdate || onFieldUpdate ? (
                <button
            ...
            {executionWorkspacesEnabled && (
              <>
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm">
                      <span>{t("project.newIssuesDefaultToIsolatedCheckout")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                    </div>
                    <div className="text-[11px] text-muted-foreground">
                      {t("project.newIssuesDefaultToIsolatedCheckoutDesc")}
                    </div>
                  </div>
                  <button
                    className={cn(
                ...
                <div className="border-t border-border/60 pt-2">
                  ...
                  >
                    {executionWorkspaceAdvancedOpen ? t("project.hideAdvancedCheckoutSettings") : t("project.showAdvancedCheckoutSettings")}
                  </button>
                </div>
                ...
                {executionWorkspaceAdvancedOpen && (
                  <div className="space-y-3">
                    <div className="text-xs text-muted-foreground">
                      {t("project.hostManagedImplementation")} <span className="text-foreground">{t("project.gitWorktree")}</span>
                    </div>
                    <div>
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t("project.baseRef")}</span>
                      ...
                      <DraftInput
                        ...
                        immediate
                        className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                        placeholder={t("project.baseRefPlaceholder")}
                      />
                    </div>
                    <div>
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t("project.branchTemplate")}</span>
                      ...
                      <DraftInput
                        ...
                        immediate
                        className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                        placeholder={t("project.branchTemplatePlaceholder")}
                      />
                    </div>
                    <div>
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t("project.worktreeParentDir")}</span>
                      ...
                      <DraftInput
                        ...
                        immediate
                        className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                        placeholder={t("project.worktreeParentDirPlaceholder")}
                      />
                    </div>
                    <div>
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t("project.provisionCommand")}</span>
                      ...
                      <DraftInput
                        ...
                        immediate
                        className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                        placeholder={t("project.provisionCommandPlaceholder")}
                      />
                    </div>
                    <div>
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{t("project.teardownCommand")}</span>
                      ...
                      <DraftInput
                        ...
                        immediate
                        className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                        placeholder={t("project.teardownCommandPlaceholder")}
                      />
                    </div>
                    <p className="text-[11px] text-muted-foreground">
                      {t("project.provisionRuns")}
                    </p>
                  </div>
                )}
              </>
