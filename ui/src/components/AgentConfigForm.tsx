...
const codexThinkingEffortOptions = [
  ...
  { id: "minimal", labelKey: "common.minimal" },
  { id: "low", labelKey: "common.low" },
  { id: "medium", labelKey: "common.medium" },
  { id: "high", labelKey: "common.high" },
] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("common.selectCompanyToCreateSecrets"));
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedCompanyId) throw new Error(t("common.selectCompanyToUploadImages"));
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("common.selectCompanyToTestAdapterEnvironment"));
      }
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agent.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agent.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label={t("agent.title")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("adapter.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "")}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agent.capabilitiesPlaceholder")}
            ...
            {isLocal && (
              <>
                <Field label={t("adapter.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("adapter.promptTemplatePlaceholder")}
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agent.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agent.adapter")}</span>
          }
          <Button
            type="button"
            variant="outline"
          ...
          >
            {testEnvironment.isPending ? t("common.testing") : t("agent.testEnvironment")}
          </Button>
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          <Field label={t("adapter.adapterType")} hint={help.adapterType}>
            <AdapterTypeDropdown
              value={adapterType}
              onChange={(t) => {
                if (isCreate) {
          ...
          {/* Working directory */}
          {isLocal && (
            <Field label={t("adapter.cwd")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agent.permissionsAndConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agent.permissionsAndConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("adapter.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              <Field label={t("adapter.bootstrapPrompt")} hint={help.bootstrapPrompt}>
                <MarkdownEditor
                  value={
                    isCreate
                      ? val!.bootstrapPrompt
                  ...
                  placeholder={t("adapter.bootstrapPromptPlaceholder")}
                  contentClassName="min-h-[44px] text-sm font-mono"
                  imageUploadHandler={async (file) => {
              ...
              <div className="rounded-md border border-sky-500/25 bg-sky-500/10 px-3 py-2 text-xs text-sky-100">
                {t("adapter.bootstrapPromptHint")}
              </div>
              {adapterType === "claude_local" && (
                <ClaudeLocalAdvancedFields {...adapterFieldProps} />
              )}

              <Field label={t("adapter.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("adapter.extraArgsPlaceholder")}
                />
              </Field>

              <Field label={t("adapter.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("adapter.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("adapter.graceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agent.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agent.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("adapter.heartbeatInterval")}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("common.sec")}
              numberPrefix={t("agent.runHeartbeatEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("agent.advancedRunPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              ...
              />
              <Field label={t("adapter.cooldownSec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("adapter.maxConcurrentRuns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation();
  const statusLabel =
    result.status === "pass" ? t("common.passed") : result.status === "warn" ? t("common.warnings") : t("common.failed");
  const statusClass =
    result.status === "pass"
...
const ADAPTER_DISPLAY_LIST: { value: string; comingSoon: boolean }[] = [
  ...AGENT_ADAPTER_TYPES.map((value) => ({
    value,
    comingSoon: !ENABLED_ADAPTER_TYPES.has(value),
  })),
];

function adapterTypeLabel(t: (key: string) => string, type: string): string {
  return t(`adapterType.${type}`);
}
      ...
      <PopoverTrigger asChild>
        <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
          <span className="inline-flex items-center gap-1.5">
            ...
            <span>{adapterTypeLabel(t, value)}</span>
          </span>
        ...
        {ADAPTER_DISPLAY_LIST.map((item) => (
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              ...
              <span>{adapterTypeLabel(t, item.value)}</span>
            </span>
            {item.comingSoon && (
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    const name = window.prompt(t("common.secretName"), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : t("agent.failedToCreateSecret"));
    }
  }
  ...
  return (
    <div className="space-y-1.5">
          ...
          <div key={i} className="flex items-center gap-1.5">
            ...
            {row.source === "secret" ? (
              <>
                ...
                >
                  <option value="">{t("common.selectSecret")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("common.createSecretFromCurrentPlainValue")}
                >
                  {t("common.new")}
                </button>
              </>
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("common.storeValueAsSecretAndReplaceWithReference")}
                >
                  {t("common.seal")}
                </button>
              </>
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("adapter.runtimeInjectedVariables")}
      </p>
    </div>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("adapter.model")} hint={help.model}>
      <Popover
        open={open}
      ...
      >
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("agent.default") : required ? t("adapter.selectModelRequired") : t("adapter.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <input
            ...
            placeholder={t("issue.searchModels")}
            value={modelSearch}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {filteredModels.length === 0 && (
              <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("issue.noModelsFound")}</p>
            )}
          </div>
        </PopoverContent>
      </Popover>
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("adapter.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected ? t(selected.labelKey) : t("common.auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
