  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyRow label={t("agent.role")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("agent.title")}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("agent.adapter")}>
          <span className="text-sm font-mono">{t(`agent.adapter${agent.adapterType.charAt(0).toUpperCase() + agent.adapterType.slice(1)}`)}</span>
        </PropertyRow>
      </div>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("agent.session")}>
