...
function getJoinTypeLabel(type: JoinType, t: ReturnType<typeof useTranslation>["t"]) {
  return type === "agent" ? t("invite.joinTypeAgent") : t("invite.joinTypeHuman");
}

function getAdapterLabel(type: AgentAdapterType, t: ReturnType<typeof useTranslation>["t"]) {
  switch (type) {
    case "claude_local":
      return t("invite.adapterLabelClaudeLocal");
    case "codex_local":
      return t("invite.adapterLabelCodexLocal");
    case "opencode_local":
      return t("invite.adapterLabelOpencodeLocal");
    case "openclaw_gateway":
      return t("invite.adapterLabelOpenclawGateway");
    case "cursor":
      return t("invite.adapterLabelCursor");
    case "process":
      return t("invite.adapterLabelProcess");
    case "http":
      return t("invite.adapterLabelHttp");
    default:
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              ...
              {onboardingInstallPath && (
                <p className="font-mono break-all">{t("invite.installTo", { path: onboardingInstallPath })}</p>
              )}
            </div>
          )}
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {t("invite.expires", { value: dateTime(invite.expiresAt, i18n.language) })}
        </p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {t("invite.joinAs", { type: getJoinTypeLabel(type, t) })}
              </button>
            ))}
          </div>
        )}
          ...
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("invite.signInOrCreate")}</Link>
              </Button>
            </div>
          </div>
        )}
