...
export function NewAgent() {
  ...
  useEffect(() => {
    if (isFirstAgent) {
      if (!name) setName(t("agent.defaultName"));
      if (!title) setTitle(t("agent.defaultTitle"));
    }
      ...
      if (!discovered.some((entry) => entry.id === selectedModel)) {
        setFormError(
          discovered.length === 0
            ...
            : `${t("onboarding.failedToLoadOpencodeModels")}: ${selectedModel}`,
        );
        return;
      }
    }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("agent.newAgent")}</h1>
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("agent.thisWillBeTheCeo")}</p>
          )}
          {formError && (
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              {createAgent.isPending ? t("agent.creating") : t("agent.createAgent")}
            </Button>
          </div>
        </div>
      </div>
