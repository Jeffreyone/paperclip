...
export async function doctor(opts: {
  ...
  try {
    ...
    const readResult: CheckResult = {
      name: "配置文件",
      status: "fail",
      message: `无法读取配置文件：${err instanceof Error ? err.message : String(err)}`,
      canRepair: false,
...
function printSummary(results: CheckResult[]): { passed: number; warned: number; failed: number } {
  ...
  const parts: string[] = [];
  parts.push(pc.green(`${passed} 通过`));
  if (warned) parts.push(pc.yellow(`${warned} 警告`));
  if (failed) parts.push(pc.red(`${failed} 失败`));

  p.note(parts.join(", "), "摘要");

  if (failed > 0) {
    p.outro(pc.red("部分检查未通过。请修复上述问题后重新运行 doctor。"));
  } else if (warned > 0) {
    p.outro(pc.yellow("所有关键检查已通过，但存在一些警告。"));
  } else {
    p.outro(pc.green("所有检查已通过！"));
  }
