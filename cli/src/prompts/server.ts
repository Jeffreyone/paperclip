    ...
    options: [
      {
        value: "local_trusted",
        label: "本地信任",
        hint: "最简单的本地配置（无需登录，仅限 localhost）",
      },
      {
        value: "authenticated",
        label: "需要认证",
        hint: "需要登录；适用于私有网络或公开部署",
      },
    ],
    ...
    const allowedHostnamesInput = await p.text({
      ...
      validate: (val) => {
        try {
          ...
          return err instanceof Error ? err.message : "主机名列表无效";
        }
      },
    });
  ...
  if (deploymentMode === "authenticated" && exposure === "public") {
    const urlInput = await p.text({
      ...
      validate: (val) => {
        ...
        if (!candidate) return "公开部署需要提供公共基础 URL";
        try {
